...
.DESCRIPTION
    Automates Stage 0–4 of the Kelivo Win7 plan:
...
.PARAMETER EngineBranch
    Flutter engine tag to build (default: 3.44.1).
.PARAMETER DartBranch
    Dart SDK tag (default: dart-3.12.1).
.PARAMETER KelivoRef
...
# Helper: run a command with visual feedback
function Run-Step {
...
Run-Step -Name "Step 4: Apply Win7 patches" -Block {
    ...
    if (-not $python) {
        # Use Python 3 from managed runtime
...
if (-not $SkipBuild) {
    Run-Step -Name "Step 5: Build Flutter Engine (release, Impeller OFF)" -Block {
        ...
        # Detect VS and set up environment
        $vsPath = & "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe" `
            -latest -products * -property installationPath
        if ($vsPath) {
            pushd $vsPath
            . "VC\Auxiliary\Build\vcvars64.bat" 2>&1 | Out-Null
            popd
        }
...
if (-not $SkipBuild) {
    Run-Step -Name "Step 7: Clone & build Kelivo Win7" -Block {
        ...
        # Also create a pubspec.win7.yaml override
        Copy-Item (Join-Path $repoRoot "dart_overrides\pubspec.win7.yaml") "pubspec.win7.yaml" -Force

        # Remove unused desktop dependencies from pubspec
...
Write-Host ""
Write-Host "To build engine patches on a new Flutter version:" -ForegroundColor White
Write-Host "  python $(Join-Path $repoRoot 'tools\apply_patches.py') --engine-dir C:\engine\src"
Write-Host "  python $(Join-Path $repoRoot 'tools\audit_engine.ps1') -EngineSrc C:\engine\src"
