<#
.SYNOPSIS
    Engine Win10+ API Audit 鈥?scan Flutter Engine + Dart SDK for Win8+ API usage.
.DESCRIPTION
    Implements the 4-entry audit methodology from plan 搂2.1.
    Run this inside the engine source tree (after gclient sync).

    Entries:
    A 鈥?grep Dart SDK runtime for known Win8+ APIs
    B 鈥?grep Flutter Engine embedder for known Win8+ APIs
    C 鈥?dumpbin analysis of built flutter_windows.dll IAT
    D 鈥?GitHub commit range analysis (optional, needs gh CLI)
...
# 鈥?detect tooling 鈥?if (-not $DumpbinPath) {
...
# 鈥?prepare output dir 鈥?$outDir = Resolve-Path (Join-Path (Get-Location) $OutDir) -ErrorAction SilentlyContinue
if (-not $outDir) {
...
# 鈥?Known Win8+ APIs to search for 鈥?$win8Apis = @(
...
if ($RunDumpbin -and $DumpbinPath -and (Test-Path $BuiltDllPath)) {
    ...
    Write-Host "=== [C] dumpbin IAT 鈥?skipped (use -RunDumpbin + -BuiltDllPath)" -ForegroundColor DarkGray
}
...
"@ -ForegroundColor DarkGray
    ...
    Write-Host "=== [D] GitHub commit range 鈥?gh CLI not available, run manually" -ForegroundColor DarkGray
}
...
$dispositions = @{
    "GetHostNameW" = "REVERT known RustDesk commit 鈫?0001-dart-revert-GetHostNameW.patch"
    "RtlAddGrowableFunctionTable" = "RESTORE Dart SDK Win7 compat branch 鈫?0002-dart-restore-RtlAddGrowableFunctionTable.patch"
    "PathCchCombineEx" = "REVERT commit 鈫?0003-dart-revert-PathCchCombineEx.patch"
    "SetFileInformationByHandle" = "DYNAMIC_LOAD GetProcAddress fallback 鈫?0007-dart-file-ops-fallback.patch"
    "GetPointerInfo" = "DYNAMIC_LOAD with WM_INPUT fallback 鈫?0005-embedder-pointer-mouse-fallback.patch"
    "EnableMouseInPointer" = "DYNAMIC_LOAD with mouse fallback 鈫?0005-embedder-pointer-mouse-fallback.patch"
    "RegisterPointerInputTarget" = "DYNAMIC_LOAD with mouse fallback 鈫?0005-embedder-pointer-mouse-fallback.patch"
    "DCompositionCreateSurfaceHandle" = "ENGINE_CONFIG enable-impeller=OFF 鈫?0004-embedder-disable-impeller-ensure-angle.patch; not a code revert"
    "GetSystemTimePreciseAsFileTime" = "NO_ACTION 鈥?has fallback in Skia/Engine"
    "SetThreadDescription" = "DYNAMIC_LOAD 鈫?0006-embedder-thread-naming-compat.patch"
    "GetThreadDescription" = "DYNAMIC_LOAD 鈫?0006-embedder-thread-naming-compat.patch"
}
...
# 鈥?Summary API found count 鈥?$foundA = $allResults | Where-Object Entry -eq "A"
